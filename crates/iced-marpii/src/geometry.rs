...
use iced_core::{self, Pixels, Point, Radians, Rectangle, Size, Svg, Transformation, Vector};
use iced_graphics::cache::{self, Cached};
use iced_graphics::color;
use iced_graphics::geometry::fill::{self, Fill};
use iced_graphics::geometry::{self, LineCap, LineDash, LineJoin, Path, Stroke, Style};
use iced_graphics::gradient::{self, Gradient};
use iced_graphics::mesh::{self, Mesh};
use iced_graphics::{Image, Text};
...
#[derive(Debug, Clone)]
///Cache entries for existing geometry.
pub struct Cache {
    pub meshes: Vec<Mesh>,
...
impl Cached for Geometry {
    ...
    fn cache(self, _group: cache::Group, previous: Option<Self::Cache>) -> Self::Cache {
        match self {
            Self::Live {
                meshes,
                images: _,
...
///A drawing frame from the marpii-backend
pub struct Frame {
...
impl Frame {
    ...
    pub fn with_clip(bounds: Rectangle) -> Frame {
        Frame {
            ...
            transforms: Transforms {
                previous: Vec::new(),
                current: Transform(lyon::math::Transform::translation(bounds.x, bounds.y)),
            },
...
///Implements the generic backend function for the frame, which makes the canvas this
///backend agnostic.
//NOTE: mostly copied from iced's wgpu implementation
impl iced_graphics::geometry::frame::Backend for Frame {
    ...
    fn center(&self) -> iced::Point {
        //NOTE: means center of relative triangle, so without
        //      the _offset_ of the bound
        iced::Point::new(self.clip_bounds.width / 2.0, self.clip_bounds.height / 2.0)
    }
    ...
    fn fill_text(&mut self, text: impl Into<geometry::Text>) {
        ...
        if self.transforms.current.is_scale_translation()
            ...
            && scale_y > 0.0
        {
            let (position, size, line_height) = if self.transforms.current.is_identity() {
    ...
    fn draw_image(&mut self, bounds: Rectangle, image: impl Into<iced_core::Image>) {
