...
use iced_marpii_shared::{spirv_std, QuadPush, ShapeCmdBuffer};
use spirv_std::glam;
...
pub fn vertex(
    ...
    *out_instance_index = instance_id;
    ...
    let (origin, extent) = cmd.bound_origin_extent();
    //calculate initical pixel-space position
...
pub fn fragment(
    ...
    let dist = cmd.distance(in_frag_coord.xy());
    ...
    if in_shadow_blur_radius > 0.0 {
        ...
        let shadow_dist = cmd.distance(in_frag_coord.xy() - in_shadow_offset);
