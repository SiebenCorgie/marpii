use marpii::{ash::vk, resources::{ComputePipeline, PushConstant, ShaderModule, FormatType}};
...
impl AlphaBlend{
    ...
    pub fn new(rmg: &mut Rmg, add: ImageHandle, add_offset: vk::Offset2D, dst: ImageHandle, dst_offset: vk::Offset2D, extent: vk::Extent2D, format_type: FormatType) -> Result<Self, anyhow::Error>{
...
impl Task for AlphaBlend{
    ...
    fn pre_record(&mut self, resources: &mut marpii_rmg::Resources, _ctx: &marpii_rmg::CtxRmg) -> Result<(), marpii_rmg::RecordError> {
        self.push_constant.get_content_mut().add =
            resources.resource_handle_or_bind(&self.add)?;
        self.push_constant.get_content_mut().dst =
            resources.resource_handle_or_bind(&self.dst)?;
