...
impl<'dta> Task for UploadImage<'dta>{
    ...
    fn pre_record(&mut self, resources: &mut crate::Resources, ctx: &CtxRmg) -> Result<(), RecordError>{
        ...
        if !desc.usage.contains(vk::ImageUsageFlags::TRANSFER_DST){
            ...
            return Err(RecordError::Any(anyhow::anyhow!("Upload Image has no transfer bit set")));
        }
    ...
    fn record(
        ...
        if let Some(bufkey) = self.host_image{
            ...
            let buffer = resources.buffer.get(bufkey).ok_or(RecordError::NoSuchResource(AnyResKey::Buffer(bufkey))).unwrap();
            let img = resources.images.get(self.target).ok_or(RecordError::NoSuchResource(AnyResKey::Image(self.target))).unwrap();
