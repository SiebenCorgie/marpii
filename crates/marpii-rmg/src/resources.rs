...
use marpii::{
    ...
    resources::{BufDesc, Buffer, Image, ImgDesc, Sampler},
};
use slotmap::SlotMap;
use std::{marker::PhantomData, sync::Arc, time::Instant};
use thiserror::Error;

use crate::{task::Attachment, RmgError, Track, TrackId, Tracks};
...
impl Resources {
    ...
    pub(crate) fn new_sampler(&mut self, sampler: Arc<Sampler>) -> SamplerHdl {
        let sampler = ResSampler { sampler };
    ...
    pub(crate) fn tmp_image(
        ...
        .map_err(|e| ResourceError::Any(e))?;
        let new = self.new_image(Arc::new(image), None);

        #[cfg(feature = "logging")]
        log::info!("Created new temporary image {:?}!", new.hdl);

        self.temporary_resources.push(TemporaryResource {
            res: AnyResource::Image(new.hdl),
    ...
    pub(crate) fn notify_new_frame(&mut self) {
        ...
        let (removed, retained) = tmp.into_iter().fold(
            (Vec::new(), Vec::new()),
            |(mut removed, mut retained), res| {
