...
use graph::{RecordError, Recorder};
use marpii::{
...
use resources::{BufferHdl, Guard, ImageHdl, Resources, SamplerHdl};
...
impl Rmg {
    ...
    pub fn new_image_uninitialized(
        &mut self,
        description: ImgDesc,
        sampler: Option<SamplerHdl>,
        name: Option<&str>,
    ) -> Result<ImageHdl, RmgError> {
        let image = Image::new(
            &self.ctx.device,
            &self.ctx.allocator,
            description,
