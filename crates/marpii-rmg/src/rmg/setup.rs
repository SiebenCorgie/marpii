...
impl Rmg {
    ///Creates a surface for `window`, assuming that you setup the device with the
    /// appropriate extensions.
    ///
    /// You'd usally use [init_for_window](Self::init_for_window) to ensure that:
    ///
    /// 1. The correct extensions are loaded
    /// 2. A graphics-capable device is selected.
    pub fn create_surface<W: HasWindowHandle + HasDisplayHandle>(
        &self,
        window: &W,
    ) -> Result<Surface, MarpiiError> {
        Surface::new(&self.ctx.instance, window).map_err(|e| MarpiiError::from(e))
    }
    ...
    pub fn init_for_window<W: HasWindowHandle + HasDisplayHandle>(
        ...
        let rmg = Self::init(Some(window), |db, _| db)?;
        Ok(rmg)
    }
    ...
    pub fn init_for_window_with<W: HasWindowHandle + HasDisplayHandle>(
        ...
        let rmg = Self::init(Some(window), on_builder)?;
        Ok(rmg)
    }
    pub fn init_headless() -> Result<Self, RmgError> {
        let rmg = Self::init::<winit::window::Window>(None, |db, _| db)?;
        Ok(rmg)
    }
    ...
    pub fn init_headless_with(
        ...
        let rmg = Self::init::<winit::window::Window>(None, on_builder)?;
        Ok(rmg)
    }
    ...
    fn init<W: HasWindowHandle + HasDisplayHandle>(
        ...
        let ctx = marpii::context::Ctx::custom_context(window, use_validation, |mut db| {
            ...
            db = db
                ...
                .with_feature(
                    vk::PhysicalDeviceVulkan12Features::default()
                        ...
                        .shader_buffer_int64_atomics(config.limit.atomics_support.any_atomic_int())
