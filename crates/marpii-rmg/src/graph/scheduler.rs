use crate::{
    resources::{BufferKey, ImageKey, Resources},
    task::Attachment,
    Rmg, TrackId,
};
...
use super::{TaskAttachment, TaskRecord};
...
pub enum SchedulerError {
    #[error(
        "Required resource could not be found. This is most likely a bug and should be reported"
    )]
    ResorceNotInTrack,
...
impl<'a> Schedule<'a> {
    pub(crate) fn from_tasks(
        ...
        while let Some(task) = tasks.pop_front() {
            ...
            let track_id = if let Some((track_idx, _track)) = schedule
                .tracks
                .iter()
                .enumerate()
                .find(|(idx, track)| track.track_id.contains(task.capability))
            {
                track_idx
            } else {
                #[cfg(feature = "logging")]
                log::error!(
                    "Could not find Queue with bit containing: {:?}",
                    task.capability
                );
            ...
            for res in task
                .buffers
                .iter()
                .map(|bufid| Res::Buffer(*bufid))
                .chain(task.images.iter().map(|imgid| Res::Image(*imgid)))
            {
    ...
    fn request_res(
        ...
        if let Some((src_track, src_track_frame)) = self.find_owner(res) {
            ...
            self.tracks[src_track].cmd_frames[src_track_frame]
                .release
                .push((dst_track_id, *res));
            ...
            if src_track_frame == self.tracks[src_track].current_frame() {
                #[cfg(feature = "logging")]
                log::info!(
                    "Finish frame {} with num acquires: {}",
                    src_track_frame,
                    self.tracks[src_track].cmd_frames[src_track_frame]
                        .acquires
                        .len()
            ...
            debug_assert!(
                self.find_owner(res) == Some((track, self.tracks.len() - 1)),
            ...
            let guard = match res {
                Res::Image(imgkey) => {
                    if let Some(img) = resources.get_image_mut(*imgkey) {
                        ...
                        #[cfg(feature = "logging")]
                        log::error!(
                            "Failed to get image for key {:?} while requesting resource.",
                            imgkey
                        );
                ...
                Res::Buffer(bufkey) => {
                    if let Some(buf) = resources.get_buffer_mut(*bufkey) {
                        ...
                        #[cfg(feature = "logging")]
                        log::error!(
                            "Failed to get buffer for key {:?} while requesting resource.",
                            bufkey
                        );
                        return Err(SchedulerError::CouldNotFindResource(*res));
                    }
            ...
            if let Some(guard) = guard {
                ...
                #[cfg(feature = "logging")]
                log::info!(
                    "Acquiring res {:?} externally from index {} to {}",
                    res,
                    src_track_index,
                    dst_track_index
                );
                ...
                assert!(
                    guard.target_val <= self.tracks[src_track_index].sem_start,
                    "Guard does not expire before this schedule starts. This is a bug!"
                );

                //make sure the first block waits at least for the given semaphore value
                self.tracks[src_track_index]
                    .cmd_frames
                    .first_mut()
                    .unwrap()
                    .add_outside_dep(guard.target_val);
                //add release to "us"
                self.tracks[src_track_index]
                    .cmd_frames
                    .first_mut()
                ...
                if self.tracks[src_track_index].current_frame() == 0 {
                    ...
                    log::info!(
                        "Finish frame {} with num acquires: {}",
                        0,
                        self.tracks[src_track_index].cmd_frames[0].acquires.len()
                    );
