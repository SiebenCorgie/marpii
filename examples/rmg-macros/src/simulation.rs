...
use marpii_rmg::{BufferHandle, ImageHandle, RecordError, Rmg, RmgError, Task, TaskUtils};
...
#[derive(TaskUtils)]
#[shader_source = "../../rmg_rendering/resources/simulation.spv"]
pub struct Simulation {
    ///Simulation buffer

    #[pc_handle = "sim_buffer"]
    pub sim_buffer: BufferHandle<SimObj>,
    #[pc_handle = "img_handle"]
...
impl Simulation {
    ...
    pub fn new(rmg: &mut Rmg) -> Result<Self, RmgError> {
        let push = PushConstant::new(
            shared::SimPush {
                sim_buffer: shared::ResourceHandle::INVALID,
                is_init: 0,
        ...
        let shader_module = ShaderModule::new_from_bytes(&rmg.ctx.device, Self::source())
...
impl Task for Simulation {
    ...
    fn pre_record(
        ...
        self.write_resource_handle(resources)?;
