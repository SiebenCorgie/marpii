...
use marpii_rmg::{Task, ImageHandle, BufferHandle, ResourceRegistry, Resources, recorder::task_scheduler::TaskSchedule};
...
impl Task for DummyTask{
    ...
    fn register(&self, registry: &mut ResourceRegistry) {
        for i in self.images.iter(){
            println!("reg: {:?}", i);
            if registry.request_image(i, vk::PipelineStageFlags2::ALL_COMMANDS, vk::AccessFlags2::empty(), vk::ImageLayout::GENERAL).is_err(){
                println!("{:?} was registered", i);
            }
        }
        for b in self.buffers.iter(){
            registry.request_buffer(b, vk::PipelineStageFlags2::ALL_COMMANDS, vk::AccessFlags2::empty()).unwrap();
        }
        println!("RegFin");
    }
...
fn main() -> Result<(), anyhow::Error> {
    ...
    let shadows_img = rmg.new_image_uninitialized(ImgDesc::texture_2d(64, 64, vk::Format::R8G8B8A8_UNORM), None).unwrap();
    let prev_post = rmg.new_image_uninitialized(ImgDesc::texture_2d(64, 64, vk::Format::R8G8B8A8_UNORM), None).unwrap();
    let lookup = rmg.new_image_uninitialized(ImgDesc::texture_2d(64, 64, vk::Format::R8G8B8A8_UNORM), None).unwrap();
    let lookup2 = rmg.new_image_uninitialized(ImgDesc::texture_2d(64, 64, vk::Format::R8G8B8A8_UNORM), None).unwrap();
    let target = rmg.new_image_uninitialized(ImgDesc::texture_2d(64, 64, vk::Format::R8G8B8A8_UNORM), None).unwrap();
    ...
    let tasks = rmg.record_compute_only()
