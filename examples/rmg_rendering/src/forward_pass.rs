use marpii::{
    ...
    ash::vk,
    context::Device,
    resources::{ComputePipeline, Image, ImgDesc, PushConstant, ShaderModule},
};
...
impl ForwardPass {
    ...
    pub fn new(rmg: &mut Rmg) -> Result<Self, RmgError> {
        ...
        let shader_module =
            ShaderModule::new_from_file(&rmg.ctx.device, "resources/forward_test.spv")?;
        ...
        let target_img = rmg.new_image_uninitialized(
            ImgDesc::storage_image_2d(1, 1, Self::FORMAT),
            Some("target img"),
        )?;
    ...
    fn flip_target_buffer(
        ...
        self.dst_img = resources.add_image(Arc::new(Image::new(
            &ctx.device,
            &ctx.allocator,
...
impl Task for ForwardPass {
    ...
    fn pre_record(
        ...
        println!(
            "Push: img: {:x}",
            self.push.get_content().target_img.index()
        );
        Ok(())
    }
    ...
    fn record(
        ...
        unsafe {
            ...
            device
                .inner
                .cmd_dispatch(*command_buffer, disp[0], disp[1], 1);
        }
    }
